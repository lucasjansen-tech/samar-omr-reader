...
def tratar_entrada(img_pil):
    """Padroniza a imagem para o OpenCV"""
    img_pil = ImageOps.exif_transpose(img_pil)
    return cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)

def alinhar_gabarito(imagem):
    """Localiza as 4 âncoras pretas nos cantos e corrige a perspectiva"""
    ...
    if len(centros) == 4:
        ...
        # Redimensiona para um padrão fixo de 800x1100
...
def extrair_respostas(alinhada):
    """Varre os 4 blocos de questões (52 questões no total)"""
